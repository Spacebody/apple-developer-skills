...
description: Review SwiftUI iOS/iPadOS UI against code-detectable Apple Human Interface Guidelines requirements, including safe areas, Dynamic Type, semantic colors and Dark Mode, SF Symbols, 44pt hit targets, standard navigation and controls, feedback states, accessibility, and internationalization. Use when the user explicitly asks for an HIG review, Apple interface-compliance check, code-level accessibility/layout audit, or whether an interface follows Apple conventions. Do not auto-use for general visual polish, branding, UX research, screenshot-first product audits, or implementation requests that are not asking for HIG compliance.
---

# Apple 人机界面指南（HIG）— 代码层 UI 审查 skill

> **平台边界：**本 skill 只用于 iOS/iPadOS。macOS、Mac Catalyst 或 iOS App on Mac 的桌面体验审查使用 `macos-hig-review`，不要把移动端 44pt 命中区等规则直接套到 Mac。
...
2. **逐条跑「检查清单」**，每条产出：
   ...
   - 修复建议（能给最小 diff 就给）
3. **可选：二次独立审查**（见末尾），发现合并去重，冲突以 HIG 正文为准。
...
仅当当前运行时规则允许且独立复核的收益明确时，使用只读子代理复核 High/Blocker 发现；否则在主线程完成复核。不要通过嵌套 `codex exec` 模拟子代理，除非用户明确批准启动独立会话。

把复核发现与本清单发现合并去重；迭代到「无新增 High/Blocker」即收敛。
